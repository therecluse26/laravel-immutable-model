...
/**
 * Represents a has-one-through relationship for immutable models.
 *
...
class ImmutableHasOneThrough
{
    ...
    /**
     * Get the related model through the intermediate model.
     *
    ...
    public function eagerLoadOnCollection(
        ...
        // Build dictionary keyed by through key (first key value = far parent's local key)
        $dictionary = [];
        foreach ($results as $relatedModel) {
            $throughKey = $this->getThroughKeyValue($relatedModel);
            // HasOneThrough: only keep the first match per parent
