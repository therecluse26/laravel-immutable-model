...
/**
 * Immutable BelongsToMany relationship.
 *
 * Extends Eloquent's BelongsToMany for full read compatibility while
 * blocking all mutation operations including pivot modifications.
 */
class ImmutableBelongsToMany extends BelongsToMany
{
    /**
     ...
     * @param bool $exists
     * @return ImmutablePivot
     */
    public function newPivot(array $attributes = [], $exists = false)
    {
        $using = $this->using;

        // Use ImmutablePivot unless a custom pivot class is specified
        $pivot = $using
            ? $using::fromRawAttributes($this->parent, $attributes, $this->getTable(), $exists)
            : ImmutablePivot::fromRawAttributes($this->parent, $attributes, $this->getTable(), $exists);
